...
searchUsrButton.addEventListener("click",function (evt) {
    ...
    if  (isValidSearchForm(name,surname,lastName,index)){
...
function isValidSearchForm(name, surname, lastName, index) {
    return name.toString().match('(^$)|(^[A-Za-zА-Яа-яёЁ]+$)') && surname.toString().match('(^$)|(^[A-Za-zА-Яа-яёЁ]+$)')
 && lastName.toString().match('(^$)|(^[A-Za-zА-Яа-яёЁ]+$)') && index.toString().match('(^$)|(^\\d{0,10}$)');
}
...
function resetUserEditForm() {
    document.querySelector('#usrEdit').reset();
}