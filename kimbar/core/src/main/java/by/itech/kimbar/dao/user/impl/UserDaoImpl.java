...
public class UserDaoImpl implements UserDao {
    ...
    private static final String FIND_USER = "SELECT * FROM client WHERE name LIKE ? OR surname LIKE ? OR last_name LIKE ? " +
            "OR gender LIKE ? OR marital_status LIKE ? OR date = ? OR country LIKE ? OR city LIKE ? " +
            "OR street LIKE ? OR house LIKE ? OR num_of_house LIKE ? OR `index` LIKE ? or citizenship LIKE ? ;";
    ...
    @Override
    public List<User> userPagination(int start, int total) throws DaoException {
    ...
    public boolean updateUser(String name, String surname, String lastName, Date date, Gender gender, String citizenship,
                              String maritalStatus, String webSite, String email, String workplace,
                              String country, String city, String street, String house, String numOfFlat,
                              Integer index, String photoPath, int idClient) throws DaoException {
        Connection c = ConnectionPool.getInstance().getConnection();
        PreparedStatement ps = null;
        int res = 0;
        try {
